"""
Writes processed sea and air pressure files in to metadata rich netCDF pressure files
"""
...
try:
    import wavelab.addons.DataTests as DataTests
    data_test_import = True
except ImportError:
    data_test_import = False
...
class NetCDFWriter(object):
    ...
    def instrument_info(self,inst, vstore):
        ...
        if inst == "Onset Hobo U20":
            ...
            vstore["instrument_level_accuracy_in_meters"] = 0.021335999
        elif inst == "LevelTroll":
            vstore["instrument_manufacturer"] = "In-Situ"
            vstore["instrument_make"] = "Level TROLL"
    ...
    def write_netCDF(self,var_datastore,series_length):
        with Dataset(self.out_filename, 'w', format="NETCDF4_CLASSIC") as ds:
            ds.createDimension("time", series_length)
            ds.createDimension("station_id", len(self.stn_station_number))
    ...
    def var_dict(self):
        ...
        global_vars = {'creator_name': self.creator_name,
                       ...
                       'sensor_orifice_elevation_at_deployment_time':  np.float64("{0:.4f}".format(
                           float(self.initial_sensor_orifice_elevation) / uc.METER_TO_FEET)),
                       'sensor_orifice_elevation_at_retrieval_time':  np.float64("{0:.4f}".format(
                           float(self.final_sensor_orifice_elevation) / uc.METER_TO_FEET)),
        ...
        if self.pressure_type == 'Sea Pressure':
            ...
            self.deployment_time = uc.convert_ms_to_datestring(self.deployment_time, pytz.utc)
            self.retrieval_time = uc.convert_ms_to_datestring(self.retrieval_time, pytz.utc)
            ...
            global_vars['initial_land_surface_elevation'] = np.float64("{0:.4f}".format(
                float(self.initial_land_surface_elevation) / uc.METER_TO_FEET))
