...
def make_timezone_aware(datetime_input, tz, daylight_savings):
    
    time_zone = pytz.timezone(tz)
    datetime_val = time_zone.localize(datetime_input, is_dst=daylight_savings)
    
    if daylight_savings is True:
        delta = timedelta(hours=1)
        datetime_val = datetime_val - delta
