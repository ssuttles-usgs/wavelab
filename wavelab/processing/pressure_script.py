#!/usr/bin/env python3
"""
This module takes arguments from the command line or the sea/baro pressure GUI
and assigns and prepares the attributest to be assigned to netCDF variables
and attributes.
"""
import sys
import time
...
def convert_to_netcdf(inputs):
    ...
    if instrument.included_baro == True:
        ...
        idx = instrument.out_filename.rfind('.csv')
        instrument.out_filename = instrument.out_filename[:idx] + 'air' + instrument.out_filename[idx:]
        time.sleep(1)
...
def process_file(args):
    ...
    if 'offset' in args:
        ...
        args['included_baro'] = args.included_baro
